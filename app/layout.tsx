...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      {/* 1. Sidebar DESKTOP (Visible uniquement sur PC) */}
      <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex-col fixed h-full z-10">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className="flex items-center gap-3 px-4 py-3 text-gray-600 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors">
              <item.icon size={20} />
      ...
      {/* 2. Barre du Haut MOBILE (Visible uniquement sur téléphone) */}
      ...
      {/* 3. Menu Déroulant MOBILE (S'ouvre quand on clique) */}
      {isMobileMenuOpen && (
      ...
      {/* 4. Contenu Principal (S'adapte automatiquement) */}
